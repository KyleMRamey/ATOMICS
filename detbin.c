            ...
            double E, double t, long id)
{
  ...
  long ebin, ubin, cbin, mbin, lbin, ibin, tbin, msh, prnt;
  ...
  if ((ptr = (long)RDB[det + DET_PTR_MESH]) > VALID_PTR)
    ni = (long)(RDB[ptr + MESH_NT]*RDB[ptr + MESH_N0]*RDB[ptr + MESH_N1]*RDB[ptr + MESH_N2]);
  else
    ni = 1;
  ...
  if ((msh = (long)RDB[det + DET_PTR_MESH]) > VALID_PTR)
    {
      ...
      else if ((ibin = MeshIndex(msh, x, y, z, -1.0)) < 0)
        return -1;
    }
