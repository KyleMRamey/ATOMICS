...
long FindLatticeRegion(long lat, long lvl, double *x, double *y, double *z,
                       long *ridx, long id)
{
  long uni, ptr, reg, n, idx, nx, ny, nz, np, tri, i, j, k, type, ncol;
    ...
    case LAT_TYPE_HY:
      {
        ...
        *y = *y - (1 - (ny % 2))*0.5*pitch;
        ...
        GetLatticeIndexes(pitch, pitch, pitch, *x, *y, 0.0, &tri, &i, &j, &k, type);
    ...
    case LAT_TYPE_YPRISM:
      {
        ...
        *z = *z - (1 - (nz % 2))*0.5*pz;
        ...
        if (type == LAT_TYPE_CUBOID)
          GetLatticeIndexes(px, py, pz, *x, *y, *z, &tri, &i, &j, &k, LAT_TYPE_S);
        else if (type == LAT_TYPE_XPRISM)
          GetLatticeIndexes(px, py, pz, *x, *y, *z, &tri, &i, &j, &k, LAT_TYPE_HX);
        else if (type == LAT_TYPE_YPRISM)
          GetLatticeIndexes(px, py, pz, *x, *y, *z, &tri, &i, &j, &k, LAT_TYPE_HY);
    ...
    case LAT_TYPE_INFHY:
      {
        ...
        *y = *y - RDB[lat + LAT_ORIG_Y0];
        ...
        GetLatticeIndexes(pitch, pitch, pitch, *x, *y, *z, &tri, &i, &j, &k, type);
