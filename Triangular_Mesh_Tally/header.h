...
typedef struct {
  ...
  MPI_Request **reqs; /* Requests to check for send completion (MPI_Isend()) */
  MPI_Status **stats; /* Statuses to get message info (MPI_Isend()) */
} DDSend;
...
typedef struct {
  ...
  long buffsize; /* Maximum buffer size */
  double *buff; /* Buffer */
  MPI_Request req; /* Request to check for receive completion (MPI_Irecv()) */
  MPI_Status stat; /* Status to get message info (MPI_Irecv()) */
} DDRecv;
